...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/09 09:46:51 by mquero            #+#    #+#             */
/*   Updated: 2025/04/09 09:46:52 by mquero           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	get_t(t_obj *cone, t_ray *ray, t_interval *interval,
		t_hit_cone_vars *var)
{
	t_point	hit;
	float	h;
...
// @brief to help the cn_hit to handle norm.
static void	hit_helper(t_hit_record *rec, t_ray *ray, t_obj *cone,
		t_hit_cone_vars *var)
{
	rec->p = ray_at(ray, rec->t);
	rec->material = &cone->material;
	var->m = vec3_dot(vec3_sub_vecs(rec->p, cone->point), cone->normal);
	var->p_base = vec3_add_vecs(cone->point, vec3_mul_vec(cone->normal,
				var->m));
	rec->normal = vec3_unit(vec3_sub_vecs(vec3_unit(vec3_sub_vecs(rec->p,
						var->p_base)), vec3_mul_vec(cone->normal, var->cos2)));
...
bool	cn_hit(t_obj *cone, t_ray *ray, t_interval *interval, t_hit_record *rec)
{
	...
	var.d_proj = vec3_sub_vecs(ray->direc, vec3_mul_vec(cone->normal,
				var.d_dot_n));
	var.oc_proj = vec3_sub_vecs(var.oc, vec3_mul_vec(cone->normal,
				var.oc_dot_n));
	var.a = vec3_dot(var.d_proj, var.d_proj) - var.cos2 * var.d_dot_n
		* var.d_dot_n;
	var.b = 2.0f * (vec3_dot(var.d_proj, var.oc_proj) - var.cos2 * var.d_dot_n
			* var.oc_dot_n);
	var.c = vec3_dot(var.oc_proj, var.oc_proj) - var.cos2 * var.oc_dot_n
		* var.oc_dot_n;
