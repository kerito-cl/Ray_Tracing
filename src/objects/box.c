...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/09 09:46:46 by mquero            #+#    #+#             */
/*   Updated: 2025/04/09 09:46:48 by mquero           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	box_hit(t_obj *box, t_ray *ray, t_interval *interval, t_hit_record *rec)
{
	...
	var.tminf = fmaxf(fmaxf(var.tmin.x, var.tmin.y), fmaxf(interval->min,
				var.tmin.z));
	var.tmaxf = fminf(fminf(var.tmax.x, var.tmax.y), fminf(interval->max,
				var.tmax.z));
