...
t_color camera_send_shadow_rays(t_info *info, t_ray *ray, t_hit_record *rec)
{
	...
	{
		ray->orig = rec->p;
		ray->direc = vec3_normalize(vec3_sub_vecs(info->lights[i].point, rec->p)); // do we have this?
...
t_color	camera_send_reflect_rays(t_info *info, t_ray *ray, t_hit_record *rec, int depth)
{
	t_ray scattered;
	t_color attenuation;

	if (rec->material->scatter(ray, rec, &attenuation, &scattered))
		return (vec3_mul_vecs(camera_ray_color(info, &scattered, info->obj, depth - 1), attenuation));
